...
const pages = [
  ...
  ['/qr-scanner/', 'qr-scanner/index.html', 'https://cleanlocaltools.com/qr-scanner/'],
...
test('homepage keeps the compact nontechnical private-local brand story',()=>{const homepage=read('index.html');for(const phrase of ['Private tools for everyday files','Useful file tools.','Your files stay with you.','Keep your files to yourself.','File is sent away','Work happens here.','Nothing is uploaded for processing.','Some files are simply too personal'])assert.ok(homepage.includes(phrase),`homepage missing brand/layout phrase: ${phrase}`);assert.ok(homepage.includes('aria-label="Typical online file tool flow"'));assert.ok(homepage.includes('aria-label="Clean Local Tools flow"'));assert.ok(homepage.includes('aria-label="Your file stays on this device"'));assert.ok(homepage.includes('id="tools"'));assert.ok(homepage.includes('id="why"'));const cards=(homepage.match(/class="tool-card"/g)||[]).length;assert.strictEqual(cards,16,'homepage should render all 16 tools in one grid');const css=read('src/styles/global.css');assert.match(css,/@media\s*\(max-width:\s*560px\)[\s\S]*\.tool-grid\s*\{\s*grid-template-columns:\s*1fr/,'mobile should collapse the tool grid to a single column showing every tool');assert.doesNotMatch(css,/\.tool-card[^{]*\{[^}]*display:\s*none/,'no tool card may be hidden at any breakpoint');const tools=homepage.indexOf('id="tools"'),why=homepage.indexOf('id="why"'),story=homepage.indexOf('aria-label="Typical online file tool flow"');assert.ok(tools>=0&&why>tools&&story>why,'homepage should put the complete tool grid before the deeper trust story');assert.doesNotMatch(homepage,/WebAssembly|PDF\.js|browser memory|server-side document processing/i,'homepage brand story should stay nontechnical');});
